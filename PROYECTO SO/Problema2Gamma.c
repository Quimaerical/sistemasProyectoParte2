...
void* vehiculoThread(void* arg) {
    ...
    if (inicio == 1) {
        // Entrada especial para subtramo 2
    ...
    for (int i = inicio; i != fin + paso; i += paso) {
        ...
        if (siguiente == 1) {
            // Para subtramo 2, verificación atómica
            pthread_mutex_lock(&subtramos[1].mutex);
            if (v->tipo == AUTO) {
                siguiente_lleno = !(subtramos[1].contadorAutos < 2 && subtramos[1].contadorCamiones == 0);
            } else {
                siguiente_lleno = (subtramos[1].vehiculosPresentes > 0);
            }
            pthread_mutex_unlock(&subtramos[1].mutex);
        } else {
        ...
        if (siguiente_lleno) {
            ...
            printf("🟡 Vehículo %d → Subtramo %d LLENO, ESPERANDO en hombrillo %d\n", 
            ...
            int puede_avanzar = 0;
            while (!puede_avanzar) {
                usleep(50000);  // Verificar cada 0.05 segundos
                
                if (siguiente == 1) {
                    pthread_mutex_lock(&subtramos[1].mutex);
                    if (v->tipo == AUTO) {
                        puede_avanzar = (subtramos[1].contadorAutos < 2 && subtramos[1].contadorCamiones == 0);
                    } else {
                        puede_avanzar = (subtramos[1].vehiculosPresentes == 0);
                    }
                    pthread_mutex_unlock(&subtramos[1].mutex);
                } else {
                    pthread_mutex_lock(&subtramos[siguiente].mutex);
